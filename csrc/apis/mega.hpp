...
static void mxfp4_mega_moe(
    ...
    DG_HOST_ASSERT(get_major_type_ab(l1_weights) == cute::UMMA::Major::K);
    DG_HOST_ASSERT(get_major_type_ab(l2_weights) == cute::UMMA::Major::K);
    // MXFP4: weights are uint8 packed E2M1 FP4 fused with their E8M0 scales,
    // so recover logical K from the tile-major scale tensor instead of the
    // storage width.
    DG_HOST_ASSERT(l1_weights.scalar_type() == torch::kUInt8);
    DG_HOST_ASSERT(l2_weights.scalar_type() == torch::kUInt8);
    DG_HOST_ASSERT(l1_weights_sf.scalar_type() == torch::kUInt8);
    ...
    // Packed weights are a flat stream of contiguous chunk-major decode tiles
    // ordered (expert, n_tile, k_block), so the shape carries a tile count
    // rather than N and K.
    ...
    const int hidden = static_cast<int>(l1_weights_sf.size(2)) * 128;
    const int intermediate_hidden = static_cast<int>(l2_weights_sf.size(2)) * 128;
    DG_HOST_ASSERT(l1_tile_bytes == kSM90MXFP4BTileBytes &&
                   l2_tile_bytes == kSM90MXFP4BTileBytes);
